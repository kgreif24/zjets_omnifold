...
def test_comp_plot_script(tmp_path):
    ...
    try:
        result = subprocess.run(
            ['python', 'run_comp_plots.py',
             '--mc', './assets/evts_000_100.root',
             '--weights', './assets/wgts.npz',
             '--pd', './assets/evts_200_300.root',
             '--store', tmp_path,
             '--use_test'],
             check=True,
             capture_output=True,
        ...
        pytest.fail(f"Script failed with return code {e.returncode} and output: {e.output}")
...
def test_test_plot_script(tmp_path):
    ...
    try:
        result = subprocess.run(
            ['python', 'run_test_plots.py',
             '--f1', './assets/evts_000_100.root',
             '--f2', './assets/evts_200_300.root',
             '--name1', 'MC',
             '--name2', 'Pseudodata',
             '--step', '1',
             '--end_weights', './assets/wgts.npz',
             '--store', tmp_path],
             check=True,
             capture_output=True,
             text=True

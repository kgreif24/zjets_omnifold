...
def test_omnifold(tmp_path):
    ...
    i2s1 = np.load(tmp_path / 'test-of' / 'test-of-run_1' / 'weights' / 'iteration_2_step_1.npz')
    ...
    assert np.all(test_weights[p190 == 1] == expected_net_weights * i1s2['test'][mc_test_p190 == 1])
    assert np.all(test_weights[p190 == 0] == i1s2['test'][mc_test_p190 == 0])

    # Check that the i2s2 weights are correct
    i2s2 = np.load(tmp_path / 'test-of' / 'test-of-run_1' / 'weights' / 'iteration_2_step_2.npz')
    ...
    assert np.all(test_weights[truth_p190 == 1] == expected_net_weights * i1s2['test'][mc_test_truth_p190 == 1])
    assert np.all(test_weights[truth_p190 == 0] == i1s2['test'][mc_test_truth_p190 == 0])

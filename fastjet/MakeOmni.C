...
void MakeOmni::Loop(){
   ...
   TFile* foutput = new TFile("out/output_omni.root", "recreate");
...
void MakeOmni::AddLundJetPlane(bool truthLevel = false) {
   /* AddLundJetPlane - This function will loop through the OmniTree, cluster all of the tracks in each event
   using the Cambridge Aachen jet clustering algorithm, and add the clustering history as a new branch
   to the tree.
   
   Arguments:
   truthLevel - If true, the function will use the truth level information in the tree. If false, it will use the reco level information.
   */

   // Check if the tree already contains the Cambridge Aachen jet clustering history, if so there is nothing to do
   

   // We want to add lund jet plane information for all of the tracks in the event. To do this we need to cluster 
   // everything in the event into a single jet. Therefore use a jet radius of 4.02 to ensure we get everything
   fastjet::JetDefinition jd(fastjet::cambridge_algorithm, 4.2);
   
   // Loop through the tree
   if (fChain == 0) return;
   int nentries = fChain->GetEntriesFast();
   for (int evt = 0; evt < nentries; nentries++) {
      ...
      if (truthLevel) {
         ntracks = ntruth_pT_tracks;
      } else {
         ntracks = nreco_pT_tracks;
      }

      // Read the tracks into a vector of PseudoJets
      vector<PseudoJet> tracks;
      for (int i = 0; i < ntracks; i++) {

         // Get track pT, eta, phi, and set mass to pion mass
         TLorentzVector constit_tlv;
         if (truthLevel) {
            constit_tlv.SetPtEtaPhiM(truth_pT_tracks[i], truth_eta_tracks[i], truth_phi_tracks[i], 0.13957);
         } else {
            constit_tlv.SetPtEtaPhiM(reco_pT_tracks[i], reco_eta_tracks[i], reco_phi_tracks[i], 0.13957);
         }

         // Build pseudojet for this track, and add to the tracks vector
         PseudoJet track_pj;
         track_pj.reset_PtYPhiM(constit_tlv.Pt(), constit_tlv.Rapidity(), constit_tlv.Phi(), constit_tlv.M());
         tracks.push_back(track_pj);

      }

      // Cluster the tracks into a single jet
      ClusterSequence cs_seq(tracks, jd);

      // Loop through cluster history and never get any funding

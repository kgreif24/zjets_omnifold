...
class UncertaintyPlotter(plotter.Plotter):
    ...
    def __init__(self, source_path, target_path, store, **kwargs):
        ...
        Fix the use_truth argument to true
        """
        super().__init__(
            source_path,
            target_path,
    ...
    def plot(self, of_weights, color="blue", recalculate=False, **kwargs):
        ...
        level MC. Only one set of weights is needed (the Omnifold weights).
        ...
        # Use glob to find all files matching the wildcard, then calculate
        # central weight over the ensemble
        ...
        t_ensemble_weights = [ens[: self.max_events] for ens in raw_ensemble_weights]
        ensemble_weights = [ens[self.source_pass190 == 1] for ens in t_ensemble_weights]
        ...
        for plot in self.plots:
            ...
            # Find sum of squared weights in each bin for source histogram to
            # get MC stat uncertainty
            ...
            rel_source_stat_uncert = np.sqrt(source_stat_var) / source_hist

            # Calculate NN init uncertainty from bootstrap weights
            var_hists = []
            # Note we need to use pre-expanded track weights for track type
            # observables
            ...
            rel_nn_init_uncert = np.sqrt(nn_init_var) / source_hist

            # Calculate total variance, total uncertainty, and ratio error
            total_var = source_stat_var + nn_init_var
            rel_total_uncert = np.sqrt(total_var) / source_hist
            ratio_err = np.sqrt(total_var) / target_hist
            ...
            rel_source_stat_uncert = np.append(
                rel_source_stat_uncert, rel_source_stat_uncert[-1]
            )
            rel_nn_init_uncert = np.append(rel_nn_init_uncert, rel_nn_init_uncert[-1])
    ...
    def _calculate_central(self, ensemble_weights):
        ...
        return np.mean(np.array(ensemble_weights), axis=0)
